...
```{r}
#get the reporting dates of cases with starting dates missing and boxplot of cases with points overlayed
crime_df%>%select(DateStart,DateReport)%>%filter(is.na(DateStart))%>%select(DateReport)->tmp1
tmp1%>%ggplot()+geom_boxplot(aes(x=1,y=DateReport))+geom_point(aes(x=1,y=DateReport),alpha=0.1)->p1
