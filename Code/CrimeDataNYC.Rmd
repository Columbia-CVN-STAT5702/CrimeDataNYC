...
### <span style="color:darkblue">Description of Data</b></span>
...
### <span style="color:darkblue">Main Analysis </b></span>
...
<span style="color:darkblue"> <b>Breakdown of crime by Level (Misdemeanors, Felonies, then Violations)  </b></span>
...
<span style="color:darkblue"> <b>Normal distribution of daily crime level of those levels  </b></span>
...
<span style="color:darkblue">Leading types of crimes  </b></span>
...
Dangerous Drugs, sixth on this list, is another category of note, particularly with the way trends in drug abuse reach the news with alacrity, and a number of states have legalized use of Marijuana in recent years.
...
<span style="color:darkblue">Borough Analysis</b></span>
...
<span style="color:darkblue"> <b>Crime by Level, Borough and Time </b></span>
...
<span style="color:darkblue"> <b>Crime over two time periods </b></span>
...
<span style="color:darkblue"> <b>Main Categories of Crime by Borough </b></span>
...
<span style="color:darkblue"> <b>Location Analysis  </b></span>
...
Violent Crimes and Dangerous Crimes also follow the same pattern as Crime overall.  


<span style="color:darkblue"> <b>Precinct differences  </b></span>
...
<span style="color:darkblue"> <b>General Decreasing Trend over 2006 to 2016 </b></span>
...
<span style="color:darkblue"> <b>Time of year and Time of day: </b></span>
...
* **Month**: We can easily see that crime peaks in the summer, or when weather is warmer, as the bar graphs for all three Levels tend to bulge from June through September. The exception is in January, which may be due to the irregular January 1 data (see above) 
...
<span style="color:darkblue"> <b>Temperature: Hotter vs. colder  </b></span>
...
d1 <- ggplot(daily_violent_df, aes(x=MaxTemp, y=CrimeCount)) + 
  ...
  annotate("text", x= 25, y=375, label=paste0("y=",round(dvlm[["coefficients"]][["MaxTemp"]],2),"x+",round(dvlm[["coefficients"]][["(Intercept)"]],0)))+
...
d2 <- ggplot(daily_drug_df, aes(x=MaxTemp, y=CrimeCount)) + 
  ...
  annotate("text", x= 25, y=300, label=paste0("y=",round(ddlm[["coefficients"]][["MaxTemp"]],2),"x+",round(ddlm[["coefficients"]][["(Intercept)"]],0)," p-value:",ddlmp))+
  ggtitle("Daily Dangerous Drug Crime Counts vs. Temperature by Level of Crime w/Linear Model") +
  scale_y_continuous(limits = c(0, 625))
...
<span style="color:darkblue"> <b>Precipitation </b></span>
...
<span style="color:darkblue"> <b>Full Moon </b></span>
...
<span style="color:grey">*Graph note: We debated (hotly) about using Pie Charts for this data. On the one hand, a bar chart would be at least equally capable of demonstrating the lack of difference, the phase of the moon has on crime. On the other hand, we felt like we had about a thousand bar charts, and we were inspired by the shape of the moon!*</span>
...
<span style="color:darkblue"> <b>Unemployment : Does lack of employment impact crime rates?</b></span>
...
ggplot(crime_e_df, aes(x=AnnAvg, y=CrimeCount)) + geom_point() +
  labs(x = "Average Annual Unemployment", y = "Total Annual Crime Incidents", title = "Effect of Unempolyment on Crime") +
...
Upon visual inspection of the annual crime rate and the unemployment rates on a scatter plot, we find no direct effect of Unemployment on crime rate.
**check below **
...
As we could anticipate from the Cleveland Dot Plots, we see that pattern in the scatterplot. There are three years of exception (lower right corner). This leads us to wonder if the economy plays a role in this finding, too.
...
# store column names
sumtabnames <- c("Type", "Overall","Bronx","Brooklyn","Manhattan","Queens","Staten Island")
...
#sumtab <-rbind(sumtab,felrow,misrow,viorow)
newrow <- rbind(vcrow,ddrow)
#newrow <- newrow[2:3,]
#vctab_df <- data.frame(vcrow)
#ddtab_df <- data.frame(ddrow)
...
### <span style="color:darkblue">Executive Summary </b></span>
...
#kable(sumtab,"html", caption="Violent Crimes and Dangerous Drugs are subcategories of the Felony, Misdemeanor and Violation categories shown. Each Borough’s Felony, Misdemeanor and Violation figures add to 100%") %>%
  #kable_styling(full_width = F, position = "float_right”) %>%
  #group_rows(“Violent Crimes and Drugs”, 4, 5)
  
  
kable(sumtab,"html") %>%
    kable_styling( full_width = F) %>%
    row_spec(4:5, italic = T, background = "#DCD0C0") %>%
    caption()
    footnote( general = " ",
              symbol = c("Above Table is auto-generated from the Crime Dataset directly")
           )
```
...
```{r echo=FALSE}
  ...
  ggplot(crime_top, aes(reorder(OffenseDesc,rel_freq), rel_freq)) +
    ...
    labs(caption="These subcategories give clearer description of what kinds of crime are most frequent") +
    theme(legend.position = "bottom",
        plot.margin = margin(15, 15, 15, 15),
        plot.caption = element_text(size = 11, family = "Garamond", color = "darkgrey", face = "italic"))
...
ggplot(crime_stat, aes(Date,rel_count,color=Level)) +
    ...
    theme_gray(14) + 
    scale_color_colorblind()  +  
    labs(caption="Misdemeanors and Felonies both end up more than 12% lower than 2006, with Felonies dropping first") +
    theme(legend.position = "bottom",
        plot.margin = margin(15, 15, 15, 15),
        plot.caption = element_text(size = 10.5,family = "Garamond", color = "darkgrey", face = "italic"))
...
ggplot(tidy_crime_w, aes(HotCold, CrimeCount)) + 
  geom_boxplot()+
  labs(x="Cold days (under 20°) vs. Hot days (over 90°)", y="Count of Crimes") +
  ggtitle("Weather Impact on Crime") +
  labs(caption="The number of crimes on Hot days is clearly higher than on Cold days") +
  theme_gray(14) + scale_color_colorblind() + 
  theme(legend.position = "bottom",
        plot.margin = margin(15, 15, 15, 15),
        plot.caption = element_text(size = 11, hjust = 0.5, family = "Garamond", color = "darkgrey", face = "italic"))
```

The phase of the moon has no impact on crime. *None.*

** Correct this line ** -> Unemployment, particularly after 2009, seems to correlate with crime.



### <span style="color:darkblue">Interactive Component </b></span>
...
* Overview  
This app provides a basic interactive feature to compare Borough Crime density across different categories like Felony, Misdemeanor and Violations. The app displays the crime density in a mosaic plot implementation. The interactive suport allows the user to choose either 2010 and 2016, and switch between the respective mosaic plots. The crime density values and borough information across each cell is displayed when the user hovers over each of them.  

Application URL -> https://bl.ocks.org/CrimeDataNyc/raw/e3362fde2a5f94aa2fa94a524742a566/0fd5a0ec59e3c563d46e770b6141535bdbe5d98f/


### <span style="color:darkblue"> <b>Conclusion </b></span>  

All our current work and analysis done in this project can be found in our Project repository on Github -> https://github.com/Columbia-CVN-STAT5702/CrimeDataNYC
